  ...
  React.useEffect(() => {
    const fetchTickets = async () => {
      try {
        ...
        const res = await fetch('https://server.apexbee.in/api/support-tickets', {
            ...
            <CardContent className="p-3.5 flex flex-col gap-2.5">
              <div className="h-40 overflow-y-auto no-scrollbar border border-border/60 bg-secondary/15 rounded-lg p-2.5 flex flex-col gap-2">
                {chatMessages.map((msg, i) => (
                  <div key={i} className={`flex flex-col max-w-[80%] text-[10.5px] rounded-lg p-2 ${msg.sender === 'user' ? 'bg-primary text-white self-end text-right' : 'bg-card text-foreground border border-border/80 self-start text-left'
                    }`}>
